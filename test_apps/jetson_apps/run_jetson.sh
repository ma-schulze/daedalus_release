...
TAS_DIR="../../test_binaries/nvidia"
BBS_DIR="../test_binaries/nvidia/bbs"
INIT_DIR="."

COMMON_OPTS=(
  --tos optee
  --no-enable-step-timeout
  --func-hook-providers gp,ftpm
)

for init_file in "$INIT_DIR"/init_bc*.py; do
  ...
  "$RUN_INIT_FUNCS" "jetson_apps/$ta_path" "${COMMON_OPTS[@]}" --hooks "jetson_apps/$init_file" --cfg-path "$cfg_path"
done
